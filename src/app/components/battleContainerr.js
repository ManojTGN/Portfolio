...
export default function BattleContainer({ image, hoverImage, header, hoveredHeader, link, className }){
    ...
    return <div onMouseEnter={()=>setIsHovered(true)} onMouseLeave={()=>setIsHovered(false)} className={`bg-neutral-900 hover:bg-neutral-800 rounded-md flex flex-col gap-2 p-3 hover:cursor-pointer hover:border-2 border-yellow-300 hover:scale-105 transition-all `+className}>
